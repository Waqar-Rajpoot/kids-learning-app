...
import { Volume2, Check, RotateCcw, ChevronRight, Home, Sparkles, Terminal } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0f172a] text-white font-display flex flex-col overflow-hidden relative">
      ...
      <header className="sticky top-0 z-50 bg-[#0f172a]/60 backdrop-blur-xl border-b border-white/5 h-20 flex items-center">
        <div className="max-w-lg mx-auto px-6 flex items-center justify-between w-full">
          ...
          <div className="text-center">
            <h1 className="text-xl font-black italic tracking-tighter uppercase">Lexical Decoder</h1>
            <div className="flex items-center justify-center gap-1.5">
              <Terminal className="w-3 h-3 text-blue-400 animate-pulse" />
              <span className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em]">Ready for input</span>
            </div>
          </div>
      ...
      <main className="max-w-md mx-auto px-6 py-6 w-full flex flex-col gap-6 relative z-10">
          ...
          {spellingWords.slice(0, 8).map((_, i) => (
            <div
              ...
              className={`h-1.5 flex-1 rounded-full transition-all duration-500 ${
                i === currentIndex ? 'bg-blue-500 shadow-[0_0_10px_#3b82f6]' : i < currentIndex ? 'bg-emerald-500/50' : 'bg-white/10'
              }`}
            />
          ))}
        </div>
        ...
        >
          ...
          {/* 5. Answer Display (Slot Machine Style) */}
          <div className="flex justify-center flex-wrap gap-2">
            {currentWord.word.split('').map((_, i) => (
              ...
              >
                {userInput[i] || ''}
                {/* Decorative UI line on filled tiles */}
                {userInput[i] && (
