                ...
                return (
                  <g key={i} onClick={() => handleDotClick(i)} className="cursor-pointer group/dot">
                    ...
                    <text
                      x={dot.x} y={dot.y + 4}
                      textAnchor="middle"
                      className={`font-black text-[10px] pointer-events-none transition-colors duration-300 ${
                        isConnected ? 'fill-white' : isNext ? 'fill-pink-400' : 'fill-white/20'
                      }`}
                    >
                      {i + 1}
                    </text>
